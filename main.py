...
class Window(kivy.uix.boxlayout.BoxLayout):
    ...
    def monitor(self, dt):
        ...
        if (self._tableLabel._mousePressPos):
            if (self._game._players[self._game._currentPlayer]._isHuman):
...
class Player:
    ...
    def playCard(self, window: Window, cards: dict, firstRound: bool, calledKing: Family) -> Card:
        time.sleep(1)
    ...
    def play(self, window: Window):
        ...
        for i in range(0, n):
            ...
            for j in range(0, self._playerNumber):
                ...
                cards[p] = self._players[p].playCard(window, cards, self._firstRound, self._calledKing)
